...
if [ "$AGENTS_HOME" != "$HOME/.agents" ] && [ -e "$HOME/.agents" ]; then
  echo "error: this is not the canonical clone and ~/.agents already exists." >&2
  echo "       Installing from here would wire hooks, the skills symlink, and the" >&2
  echo "       launchd job to THIS checkout (and duplicate the settings hooks," >&2
  echo "       whose dedup compares exact paths). Run ~/.agents/install.sh instead." >&2
  exit 1
fi
...
EOF
  ...
  if jq -e --arg p "$hook_path" 'any(.hooks.SessionEnd[]?; (.hooks[]?.args[]?) == $p)' \
    ...
    cp "$settings" "$settings.bak.$STAMP.sessionend"
    tmp="$(mktemp)"
    if jq --arg p "$hook_path" '
      ...
      mv "$tmp" "$settings"
      echo "merged SessionEnd hook into $settings (backup: $settings.bak.$STAMP.sessionend)"
    else
      rm -f "$tmp" "$settings.bak.$STAMP.sessionend"
...
if [ -f "$settings" ] && command -v jq >/dev/null 2>&1; then
  if jq -e '.hooks.SessionStart[]?.hooks[]?.args[]? | select(contains("sync.sh"))' "$settings" >/dev/null 2>&1; then
    ...
    # distinct suffix: the SessionEnd block above may have backed up in this
    # same run, and a shared name would clobber the true pre-install state
    cp "$settings" "$settings.bak.$STAMP.sessionstart"
    tmp="$(mktemp)"
    if jq --arg cmd "$sync_start_cmd" '
      ...
      ' "$settings" > "$tmp" 2>/dev/null; then
      mv "$tmp" "$settings"
      echo "wired SessionStart sync-check into $settings (backup: $settings.bak.$STAMP.sessionstart)"
    else
      rm -f "$tmp" "$settings.bak.$STAMP.sessionstart"
